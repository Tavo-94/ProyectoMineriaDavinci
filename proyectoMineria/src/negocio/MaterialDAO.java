...
public class MaterialDAO {
	...
	public DefaultTableModel modeloParaJTable() {
		try {
			...
			// id deposito hardcodeado a 1 porque hay un solo deposito
			String query = "SELECT * FROM material WHERE deposito_iddeposito = 1;";
	...
	// elimino los registros necesarios de la tabla material en funcion de la
	// cantidad y la pureza del material comprado
	public void eliminarMaterialCompradoDeLaTablaMaterial(Material materialComprado) {
		...
		for (Material material : listaFiltradaPorPureza) {
			if (material.getCantidad() <= materialComprado.getCantidad()) {
				...
				// actualizo la cantiadad de material comprado restandole lo que ya elimine de
				// la base de datos
				...
				// si la cantidad del registro es mayor a la cantiadad de material comrprado
				// entonces...
				// Realizo un update de este registro en su columna cantidad restandole la
				// cantidad del material pedido
